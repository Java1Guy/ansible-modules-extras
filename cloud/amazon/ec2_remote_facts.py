...
class Ec2FactInventory(object):
    ...
    def run(self):
        # Cache
        if self.module.params['refresh_cache']:
            self.do_api_calls_update_cache()
        elif not self.is_cache_valid():
            self.do_api_calls_update_cache()

        # Data to print
        if 'host' in self.module.params and self.module.params['host']:
            data_to_print = self.get_host_info(self.module.params['host'])
...
def main():
    ...
    argument_spec.update(dict(
        refresh_cache={'required': False, 'type': 'bool' },
        host={'required': False, 'type': 'str' }
    ))

    module = AnsibleModule(argument_spec=argument_spec, supports_check_mode=True)

    if 'refresh_cache' not in module.params:
    	module.params['refresh_cache'] = False

    ec2_facts = Ec2FactInventory(module).run()
    if '_meta' in ec2_facts:
    	ec2_facts = ec2_facts['_meta']
    if 'hostvars' in ec2_facts:
    	ec2_facts = ec2_facts['hostvars']
    ec2_facts_result = dict(changed=False, hostvars=ec2_facts)
